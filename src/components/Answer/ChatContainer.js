			...
			return (
				<>
					<div className="answer-grid-items">
						...
						{console.log(
							"=======================",
							userData.upVotedAnswers
						)}
						<div className="votes">
							{userData.upVotedAnswers.includes(answer?._id) ? (
								<img
									src={upVoteImg}
							...
							{userData.downVotedAnswers.includes(answer?._id) ? (
								<img
									src={downVoteImg}
									className="vote-icon voted"
									id={`down${answer?._id}`}
		...
		ApiGet(`answer/vote/id=${answer?._id}`)
			.then((res) => {
				if (res.data.payload.voted === -1) {
					up += 1;
					down -= 1;
					userData.upVotedAnswers.push(answer?._id);
					const i = userData.downVotedAnswers.indexOf(answer?._id);
					if (i > -1) {
						userData.downVotedAnswers.splice(i, 1);
					}
				}
				...
				ApiPut(`answer/upVote/id=${answer?._id}&downVote=${down}&upVote=${up}`)
					.then((res) => {
						if (res.data.result === 0) {
		...
		ApiGet(`answer/vote/id=${answer?._id}`)
			.then((res) => {
				if (res.data.payload.voted === 1) {
					down += 1;
					up -= 1;
					const i = userData.upVotedAnswers.indexOf(answer?._id);
					if (i > -1) {
						userData.upVotedAnswers.splice(i, 1);
					}
					userData.downVotedAnswers.push(answer?._id);
				}
				if (res.data.payload.voted === 0) {
					down += 1;
					userData.downVotedAnswers.push(answer?._id);
				...
				ApiPut(
					`question/downVote/id=${answer?._id}&downVote=${down}&upVote=${up}`
				)
					.then((res) => {
						if (res.data.result === 0) {
							// fetchQuestionIHaveReceived(true);
